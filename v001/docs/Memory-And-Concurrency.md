# Memory & concurrency validation (valgrind) — v001

A record of the validation pass over the Qlite v001 wrapper using **valgrind** — memcheck
for memory safety, and helgrind + DRD for data races where concurrent use is in scope. The
[v001 README](../README.md) is the overview; this documents what was run, what it found,
what was fixed, and — honestly — what it does **not** cover.
...
```bash
# one-time
sudo apt-get install -y valgrind     # or: dnf install valgrind

# memcheck a test binary (cwd = scratch dir, so any DB file stays out of the repo)
cd /tmp/scratch
valgrind --leak-check=full --error-exitcode=1 /path/to/qlite_v001_test
```
