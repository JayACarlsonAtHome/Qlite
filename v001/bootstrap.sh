#!/usr/bin/env bash
#
# Qlite v001 bootstrap — the thin, bare-metal entry point (C++23 world).
#
...
#   4. Otherwise -> cmake configure + build + ctest. Add --modules for the C++23 module build.
#
...
if [ -n "$missing" ]; then
  echo "Baseline missing:$missing"
  {
    ...
    echo "# Installs the baseline needed to build Qlite v001 on family: $family"
...
if [ "$WANT_MODULES" = "1" ]; then
  ...
  echo "=== configuring (with C++23 modules) ==="
else
...
echo
echo "Bootstrap complete. Qlite v001 built and tested in: $BUILD"
