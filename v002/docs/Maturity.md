# Maturity — an honest self-assessment (v002)

Where Qlite v002 actually sits, on a 12-level scale, and why. Like the rest of these docs
this is candid: the goal is an accurate read, not a flattering one. The
[v002 README](../README.md) is the overview. v002 is the **C++26** frontier (see
[Cpp26_Adoption.md](Cpp26_Adoption.md)), so part of its maturity story is "how settled is
the bleeding edge it stands on."
...
## Where Qlite v002 sits

_TBD — to be filled once the test infrastructure and first matrix run exist._

The honest split to assess is **craft vs. structure**, plus a **frontier caveat**:

- **Craft** — the single-body/two-front-end design, the four additive features over a
  frozen surface, the C++26 safety adoptions (contracts shim, bounds/saturation), the
  shared-runner anti-drift discipline. _Rung: TBD._
- **Structure** — solo author, no external CI by design (the local runner *is* the CI),
  results committed into the repo. _Rung: TBD._
- **Frontier caveat** — v002 stands on **experimental** ground (C++26, a contracts *shim*
  not native contracts, modules + `import std;`). That's deliberate and valuable as a
  toolchain example, but it caps "settledness" until the standard and tools catch up.

## What would move the needle

- **Up to professional (5–7):** the `Qlite_test_cli` matrix landing, a recorded results
  run, the valgrind gate wired in, contracts shim migrating cleanly toward native P2900.
...
- v002 rides experimental C++26 toolchains; the contracts are a **shim**, not native.
