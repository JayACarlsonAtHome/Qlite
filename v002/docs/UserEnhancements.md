...
v002 is the **C++26** world, so some ideas here lean on C++26 features (see
...
4. **Compile-time SQL checks.** With C++26 in play, explore validating bind/column counts
   (and parameter names) against the SQL at compile time.
5. **Native contracts at the API boundary.** When `__cpp_contracts` lands, promote the shim
   `QLITE_PRE/POST` checks to native `pre`/`post` on the public methods — broader contract
   coverage with first-class diagnostics. See [Cpp26_Adoption.md](Cpp26_Adoption.md).
